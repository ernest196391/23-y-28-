...
export default function Support({onAdd,cart={}}){
  ...
  async function ask(text,attachments=files){
    ...
    try{
      ...
      if(!r.ok)throw Error(data.error||"Veci no pudo responder ahora.");
      if(data.action==="add"&&Array.isArray(data.products)&&data.products.length){data.products.forEach(p=>onAdd(p.id));data.answer=(data.answer||"")+` ${data.products.length===1?"Ya lo añadí":"Ya añadí los productos"} al carrito.`}
      setMessages(m=>[...m,{role:"assistant",text:data.answer,products:data.products}]);setQ("");setFiles([]);
  ...
  const avatar="/brand/veci-avatar.svg?v=3";
  ...
  {open&&<div className="bessyBackdrop" onClick={close}><section ref={panel} tabIndex={-1} role="dialog" aria-modal="true" aria-label="Veci" className="bessyPanel" onClick={e=>e.stopPropagation()} onKeyDown={keys}>
    ...
    {(recording||voiceStatus)&&<div className="veciListening" role="status"><span className={recording?"veciListeningDot":""}/><div><b>{voiceStatus||"Escuchando…"}</b><small>{recording?"Sigue hablando. Veci no enviará nada hasta que pulses la flecha.":"Puedes corregir el texto antes de enviarlo."}</small></div>{recording&&<button type="button" onClick={dictate}>Detener</button>}</div>}
