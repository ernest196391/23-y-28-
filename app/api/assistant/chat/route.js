...
let text="",lastError=null;for(const [name,fn] of providers){try{text=await fn();if(text)break}catch(e){lastError=e;console.error("[Veci/provider-fallback]",name,e.message)}}
if(!text){console.error("[Veci/all-providers-failed]",lastError?.message||"unknown");return json({error:"Veci no pudo conectarse a su motor de IA ahora. Inténtalo nuevamente en unos segundos."},503)}
...
const action=parsed.action==="add"?"add":parsed.action==="contact"?"contact":"show";
const contact=action==="contact"?{phone:business.whatsapp,url:"https://wa.me/"+business.whatsapp}:null;
return json({answer:parsed.answer.slice(0,3000),products:selected,action,contact});
}catch(e){console.error("[Veci/unhandled]",e?.message||e);return json({error:"No pude responder ahora. Tu consulta no se ha convertido en un pedido. Inténtalo de nuevo."},503)}
}
