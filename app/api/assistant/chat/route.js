...
if(!parsed||typeof parsed.answer!=="string"||!parsed.answer.trim()){parsed={answer:cleaned.slice(0,3000)||"No pude responder esa consulta.",productIds:[]}}
const ids=new Set(Array.isArray(parsed.productIds)?parsed.productIds.map(Number):[]);
return json({answer:parsed.answer.slice(0,3000),products:products.filter(p=>ids.has(Number(p.id))).slice(0,3)});
